...
注释：这非常重要，任何命名空间错误、遗漏或缺失将会造成未知的错误（缺失映射、材质丢失、文件失效等），所以在加载前请务必认真检查是否存在从下文章节复制示例代码后忘记更改的`tutorial`命名空间
...
```json
{
  ...
  "dependencies": {
    "tacz": "[1.1.4,)"
  // 注释：以上代码意味着需要1.1.4版本（包括）及往后版本的tacz
  // 注释：显然，你需要手动删除这些注释以确保json文件正确加载
  }
}
```
...
注释：即其中需包含有且仅有一个命名空间文件夹，一切资源存放需要在`assets/tutorial`路径下
...
注释：其中也需包含有且仅有一个命名空间文件夹，一切数据存放需要在`data/tutorial`路径下
...
声明：所有注释仅代表个人看法，如有冲突请以官方教程为准（特别注明除外），如有谬误请加反馈群693967725，欢迎批评斧正
